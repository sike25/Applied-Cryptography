...
with open(statefile, 'rt') as sf:
    ...
    rcvsqn = int(sf.readline()[len("sndsqn: "):], base=10) # type should be integer
...
sndsqn = int.from_bytes(header_sqn, byteorder='big') # the value of sqn in the header, should be of type integer 
if (sndsqn <= rcvsqn):
...
try:
    ...
    payload = Padding.unpad(padded_payload, AES.block_size)
